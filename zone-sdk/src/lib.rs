...
use logos_blockchain_zone_sdk::{
    ...
    node_types::{
        ...
        ProcessedBlockEvent, SignedOps, Slot, TimeInfo, Unverified, WalletFundRequestBody,
...
const STREAM_IDLE_CHECK: Duration = Duration::from_secs(1);
...
impl NodeModuleClient {
        ...
        Dec: Fn(&EventData) -> Result<StreamItem<T>, ModuleError> + Send + 'static,
    {
        ...
        let (tx, rx) = mpsc::channel::<T>(STREAM_CAPACITY);
        std::thread::Builder::new()
            .name(format!("zone-sdk-{event}"))
            .spawn(move || {
                while !tx.is_closed() {
                    let ev = match subscription.receiver().recv_timeout(STREAM_IDLE_CHECK) {
                        Ok(ev) => ev,
